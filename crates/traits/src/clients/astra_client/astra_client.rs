use crate::{nosql::interface::{CqlStore, NoSql}, query::query::{QueryInterface,FindOne, Create, QueryError}, data_types::types::{IntoValue,CqlType, ToCqlRow}};
...
impl <T: NoSql + Send> QueryInterface<stargate_grpc::StargateClient> for FindOne<T> {
    async fn execute(self, store : &mut stargate_grpc::StargateClient) -> Result<Self::Output, QueryError> {
        let statement = <Self as QueryInterface<stargate_grpc::StargateClient>>::into_statement(self);
